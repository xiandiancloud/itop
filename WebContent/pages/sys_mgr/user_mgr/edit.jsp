...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<div class="mianbaoxie">
			当前位置：<a href="#1">系统管理</a> > <a href="${ctx}/deptMgr">部门管理</a>> <a href="${ctx}/userMgr">用户列表</a>  > 编辑
		</div>
		<form:form name="mainForm" id="mainForm" method="post" action="${ctx}/userMgr/update"  modelAttribute="resultObject">
			...
			<table width="100%" class="table_border">
				...
				<tr>
					...
					<td>
						...
						<input type="text" name="newLoginPwd" id="newLoginPwd" class="form_input {maxlength:16,minlength:6}" size="20"/>
					</td>
				</tr>
				<tr>
					<td class="title" width="100">
					...
					<td>
						<input type="password" name="reLoginPwd" class="form_input {equalTo:'#newLoginPwd'}" size="20"/>
					</td>
				</tr>
				<tr>
					<td class="title" width="100">
				...
				<tr>
					...
					<td>
						<form:radiobutton path="validTag" onclick="$('#validDateTr').hide();$('#validDate').val('')" value="1" cssClass="required"/>是
						<form:radiobutton path="validTag"  onclick="$('#validDateTr').show()" value="0" cssClass="required"/>否
					</td>
				</tr>
	...
	$(function(){
		$('#mainForm').validate({
			submitHandler: function(form) {
				...
				if($("#loginPwd").val() == "" || $("#loginPwd").val() == null){
					$("#loginPwd").val('${resultObject.loginPwd}');
				}
			 	form.submit();
	 		}
		});
		...
		//改变是否永久生效时来确定是否显示有效日期
		function changeValidTag(validTag)
		{
			if(validTag == '1')
			{
				$("#validDateTr").hide();
		...
		function init()
		{
			var validTag = '${resultObject.validTag}';
			changeValidTag(validTag);
			var roleIds = '${roleIds}';
			setCheckboxCheckedValue("roleIds",roleIds);
			var validDate = '${resultObject.validDate}';
			if(validDate != null && validDate != ""){
				$("#_validDate").val(validDate.substr(0,4)+"-"+validDate.substr(4,2)+"-"+validDate.substr(6,2));
			}
			$("#deptId").val('${resultObject.sysDept.deptId}');
		}
		init();
	</script>
</html>