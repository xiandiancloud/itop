...
}, jQuery.validator.format("请输入大于前面的值"));

jQuery.validator.addMethod("greatThanEqual", function(value, element, params) {
	var target = $(params);
	return this.optional(element) || value>=target.val();
}, jQuery.validator.format("请输入大于等于前面的值"));

jQuery.validator.addMethod("lessThan", function(value, element, params) {
	var target = $(params);
	return this.optional(element) || value<target.val();
}, jQuery.validator.format("请输入小于前面的值"));
...
//当前输入框日期大于等于当前时间
jQuery.validator.addMethod("greatThanEqualToday", function(value, element) {
	 var d = new Date(); 
	 var year = d.getYear(); 
