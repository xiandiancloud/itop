...
Validator.messageSource['en-us'] = [
	...
	['validate-one-checked','Please checked one of the above options.'],
	//中国特有的相关验证提示信息
	...
	['validate-il-character','Input value cannot include illegal character,For example "\'\\<>\*\&'],
	['validate-cannot-chinese','Input value cannot include chinese']
]
...
Validator.messageSource['zh-cn'] = [
	...
	['validate-currency-dollar' , 'Please enter a valid $ amount. For example $100.00 .'],
	['validate-one-required' , '前面选项至少选择一个.'],
	...
	['validate-mobile-phone-yd','请输入正确的移动手机号码,当前长度为%s.'],
	['validate-zip','请输入有效的邮政编码'],
	['validate-qq','请输入有效的QQ号码.'],
	['validate-il-character','输入的字符不能包含非法字符,如"\'\\<>\*\&'],
	['validate-cannot-chinese','不能包含中文']
]
...
Validation.addAllThese([
	...
	//chekcBox 至少选择一个 add by  yao
	['validate-one-checked', function (v,elm) {
				var checkboxs = document.getElementsByName(elm.name);
				return $A(checkboxs).any(function(elm) {
					return $F(elm);
				});
			},{ignoreEmptyValue : false}],		
	...
	['validate-phone',/^((0[1-9]{3})?(0[12][0-9])?[-])?\d{6,11}$/],
	['validate-mobile-phone',/(^0?[1][358][0-9]{9}$)/],
	['validate-mobile-phone-yd',/^(134|135|136|137|138|139|150|151|152|158|159){1}\d{8}$/],
	['validate-zip',/^[1-9]\d{5}$/],
	['validate-qq',/^[1-9]\d{4,8}$/],
	['validate-il-character',/^[^"'\\<>\*&]+$/],
	['validate-cannot-chinese',/^[^\u4e00-\u9fa5]+$/]
]);
